# Thrive Website — Page Build & SEO Tracker

อัปเดตล่าสุด: 2026-05-17
ผู้รับผิดชอบ: Thrive Wellness Team + AI
Repository: https://github.com/achotirat/thrive-website

---

## สถานะภาพรวม

| Phase | สถานะ | หมายเหตุ |
|-------|-------|---------|
| Phase 0 — Security & Foundation | ✅ เสร็จ | Netlify, DNS, API health endpoint พร้อม |
| Phase 1 — Astro Bootstrap | ⏳ ยังไม่เริ่ม | รอ scaffold Astro project |
| Phase 2 — Tier A Pages | 🔄 กำลังทำ | Priority 1 ✅ ครบ · Priority 2 เหลือ /contact |
| Phase 3 — Ads Handover | ⏳ Deadline 1 Jun 2026 | |
| Phase 4 — Lead Inbox | ⏳ ยังไม่เริ่ม | |
| Phase 5 — Blog Migration | ⏳ ยังไม่เริ่ม | |

---

## Doctor on File

**พญ. ชนากานต์ ตระหง่านศรี (หมอนุ่น)**
Chanakan Trangansri, MD.

Specializations: Anti-aging & Regenerative Medicine · Nutrition Wellness · Lifestyle Medicine · Hormone Balance · Weight Management · Chelation & Detoxification

| ไฟล์รูป | ขนาด | ใช้ใน |
|---------|------|------|
| `dr-chanakan-trangansri-thrive-400x400.jpg` | 400×400 | Doctor section ทุกหน้า |
| `dr-chanakan-trangansri-thrive-800x600.jpg` | 800×600 | About page |

JSON-LD `@id`: `https://www.thrivewellnessth.com/#dr-chanakan`

**กฎ:** ทุกครั้งที่สร้างหรืออัปเดตหน้าที่มีโปรไฟล์หมอนุ่น ต้องอัปเดตตารางนี้ทันที ก่อน commit

| หน้าที่ใส่ข้อมูลหมอนุ่นแล้ว | วันที่ |
|--------------------------|-------|
| `fit132.mdx` (Astro service page) | 2026-06-09 |
| `epispan.mdx` (Astro service page) | 2026-07-02 |
| `food-intolerance.html` | 2026-05-08 |
| `adrenal-fatigue.html` | 2026-05-08 |
| `hormones-quiz.html` | 2026-05-09 |
| `hbot.html` | 2026-05-09 |
| `iv-drip.html` | 2026-05-09 |
| `gluta.html` | 2026-05-09 |
| `chelation.html` | 2026-05-09 |
| `personalized-vitamins.html` | 2026-05-09 |
| `nad-plus.html` | 2026-05-09 |
| `nk-cell.html` | 2026-05-09 |
| `allergy-ige.html` | 2026-05-09 |
| `urine-organic-test.html` | 2026-05-09 |
| `vitamin-d.html` | 2026-05-09 |
| `vitamins-and-micronutrients.html` | 2026-05-09 |
| `check-up.html` | 2026-05-09 |
| `about.html` | 2026-05-09 |
| `oligoscan.html` | 2026-05-09 |
| `cancer-screening.html` | 2026-05-09 |
| `dna-test.html` | 2026-05-09 |
| `blog-immune-system.html` | 2026-05-15 |
| `blog-growth-hormone-decline.html` | 2026-05-15 |
| `blog-mood-swings.html` | 2026-05-15 |
| `blog-herbal-compress-massage.html` | 2026-05-15 |
| `blog-vitamin-c-guide.html` | 2026-05-15 |
| `blog-allergy-ige.html` | 2026-05-15 |
| `blog-magnesium.html` | 2026-05-15 |
| `blog-skin-immunity.html` | 2026-05-15 |
| `blog-vitamin-b6.html` | 2026-05-15 |
| `blog-gut-3p.html` | 2026-05-15 |
| `blog-steroid-cortisol.html` | 2026-05-15 |
| `blog-pid.html` | 2026-05-15 |
| `blog-menorrhagia.html` | 2026-05-16 |
| `blog-vitamin-b3.html` | 2026-05-16 |
| `blog-8-healthy-characteristics.html` | 2026-05-16 |
| `blog-testosterone-women.html` | 2026-05-16 |
| `blog-preservatives.html` | 2026-05-16 |
| `blog-check-immune-system.html` | 2026-05-16 |
| `blog-female-hormones.html` | 2026-05-16 |
| `blog-hgh-natural-boost.html` | 2026-05-16 |
| `blog-kombucha-benefits.html` | 2026-05-16 |
| `blog-syncope-fainting.html` | 2026-05-16 |
| `blog-sleepwalking.html` | 2026-05-16 |
| `blog-burnout-adrenal.html` | 2026-05-16 |
| `blog-nk-cell.html` | 2026-05-16 |
| `blog-progesterone.html` | 2026-05-16 |
| `blog-menopause-hormones.html` | 2026-05-17 |
| `blog-food-intolerance-igg.html` | 2026-05-17 |
| `blog-pms.html` | 2026-05-17 |
| `blog-pcos.html` | 2026-05-17 |
| `blog-insulin.html` | 2026-05-17 |
| `blog-adrenal-fatigue-post.html` | 2026-05-17 |
| `blog-coq10.html` | 2026-05-17 |
| `blog-vitamin-d-immune.html` | 2026-05-17 |
| `blog-abnormal-period.html` | 2026-05-17 |
| `blog-mental-health.html` | 2026-05-17 |
| `blog-chili.html` | 2026-05-17 |
| `blog-minerals.html` | 2026-05-17 |
| `blog-apple-benefit.html` | 2026-05-17 |
| `blog-periodpain.html` | 2026-05-17 |
| `blog-zinc.html` | 2026-05-17 |
| `blog-immunity.html` | 2026-05-17 |
| `blog-growth-factor.html` | 2026-05-17 |
| `blog-smiling-depression.html` | 2026-05-17 |
| `blog-glutathione.html` | 2026-05-17 |
| `blog-ashwagandha.html` | 2026-05-17 |
| `blog-intestine.html` | 2026-05-17 |
| `blog-tryptophan.html` | 2026-05-17 |
| `blog-gaba.html` | 2026-05-17 |
| `blog-silica.html` | 2026-05-17 |
| `blog-bromelain.html` | 2026-05-17 |
| `blog-omega3.html` | 2026-05-17 |
| `blog-neurotransmitters.html` | 2026-05-17 |
| `blog-period-acne.html` | 2026-05-17 |
| `blog-probiotic-foods.html` | 2026-05-17 |
| `blog-vitamin-a.html` | 2026-05-17 |
| `blog-l-carnitine.html` | 2026-05-17 |
| `blog-menstrual-pain.html` | 2026-05-17 |
| `blog-urticaria-hives.html` | 2026-05-17 |
| `blog-triglyceride.html` | 2026-05-17 |
| `blog-chromium.html` | 2026-05-17 |
| `blog-clogged-arteries.html` | 2026-05-18 |
| `blog-love-neurotransmitter.html` | 2026-05-18 |
| `docs/blog/hairloss.md` (Sanity pipeline, doctor quote) | 2026-08-02 |

---

**นายแพทย์พิจักษณ์ วงศ์วิศิษฎ์ (หมอบาย)**
Dr. Pijak Wongvisit, MD. — Preventive & Regenerative Medicine

Specializations: Preventive Medicine · Hormone Balance · Weight Management · Genetic Counseling · Regenerative Medicine

| ไฟล์รูป | ขนาด | ใช้ใน |
|---------|------|------|
| `dr-pijak-wongvisit-thrive-400x400.jpg` | 400×400 | Doctor section / Team card |

JSON-LD `@id`: `https://www.thrivewellnessth.com/#dr-pijak`

**กฎ:** ทุกครั้งที่สร้างหรืออัปเดตหน้าที่มีโปรไฟล์หมอบาย ต้องอัปเดตตารางนี้ทันที ก่อน commit — ตั้งแต่ 2026-08-04 ผู้ใช้กำหนดให้หมอบายเป็นแพทย์ที่อ้างอิงใน blog post ทุกฉบับ (แทนหมอนุ่น)

| หน้าที่ใส่ข้อมูลหมอบายแล้ว | วันที่ |
|--------------------------|-------|
| `longevity-check-up.mdx` (Astro service page) | 2026-07-31 |
| `about.astro` (Team section) | 2026-07-31 |
| `blog/hyrox-overtraining-hormone-imbalance` (Sanity blog post — quote + Reviewed-by card) | 2026-08-05 |
| `blog/red-s-relative-energy-deficiency-sport` (Sanity blog post — quote + Reviewed-by card) | 2026-08-05 |
| `blog/low-testosterone-heavy-exercise-men` (Sanity blog post — quote + Reviewed-by card) | 2026-08-05 |
| `blog/prevent-hormone-imbalance-heavy-training` (Sanity blog post — quote + Reviewed-by card) | 2026-08-05 |

---

## Clinic Contact (ใช้ข้อมูลนี้ทุกหน้า)

```
โทร: +66959349640 (tel:+66959349640)
LINE@: @thrivewellnessth (https://line.me/R/ti/p/@thrivewellnessth)
ที่อยู่: The Crystal Park (ชั้น 2) อาคาร B เลขที่ 199 ถนนประดิษฐ์มนูธรรม ลาดพร้าว กรุงเทพฯ 10230
เวลา: เปิดทุกวัน 10:00–19:00
```

---

## หน้าที่สร้างแล้ว

### Tier A — Priority 1

| URL | Views | สถานะ |
|-----|------:|-------|
| `/food-intolerance` | 3,903 | ✅ เสร็จ |
| `/adrenal-fatigue` | 1,421 | ✅ เสร็จ |
| `/hormones-quiz` | 1,344 | ✅ เสร็จ |
| `/hbot` | 705 | ✅ เสร็จ |
| `/iv-drip` | 492 | ✅ เสร็จ |
| `/` (homepage) | 465 | ⏳ ยังไม่เริ่ม |
| `/gluta` | 252 | ✅ เสร็จ |
| `/chelation` | 187 | ✅ เสร็จ |
| `/personalized-vitamins` | 148 | ✅ เสร็จ |
| `/nad` | 120 | ✅ เสร็จ |
| `/nk-cell` | 116 | ✅ เสร็จ |

### Tier A — Priority 2

| URL | Views | สถานะ |
|-----|------:|-------|
| `/fit132` | ranking #1 Wix | ✅ Astro เสร็จ (2026-06-09) — MDX + page |
| `/allergy-ige` | 74 | ✅ เสร็จ |
| `/urine-organic-test` | 55 | ✅ เสร็จ |
| `/vitamin-d` | 47 | ✅ เสร็จ |
| `/doctors` | 45 | ⏳ ข้ามไปก่อน |
| `/vitamins-and-micronutrients` | 42 | ✅ เสร็จ |
| `/check-up` | 38 | ✅ เสร็จ |
| `/about` | 35 | ✅ เสร็จ |
| `/oligoscan` | 25 | ✅ เสร็จ |
| `/cancer-screening` | 24 | ✅ เสร็จ |
| `/dna-test` | 18 | ✅ เสร็จ |
| `/contact` | 4 | ⏳ ข้ามไปก่อน |

---

## รายละเอียดแต่ละหน้า

---

### `/food-intolerance` — food-intolerance.html
สร้าง: 2026-05-08 | Priority: Tier A #1 | Views: 3,903 | Theme: teal

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack: title, description, canonical, robots
- [x] Open Graph (7 tags) + Twitter Card (4 tags)
- [x] GEO tags (4) + Hreflang (th-TH, en, x-default)
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "ท้องอืด ผิวลาย เหนื่อยตลอด ไม่ใช่เรื่องปกติ"
- [x] Section: ภูมิแพ้อาหารแฝงคืออะไร
- [x] Section: อาการ (symptom grid 6 items)
- [x] Section: สาเหตุ + อาหารที่พบบ่อย
- [x] Section: IgE vs IgG comparison table
- [x] Section: วิธีตรวจ 4 ขั้นตอน
- [x] Section: Package IgG 216 ชนิด + CTA
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references (4 PubMed)
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `food-intolerance-igg-hero-1200x630.jpg`
- [x] Doctor photo: `dr-chanakan-trangansri-thrive-400x400.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/Price range
- [ ] รายชื่ออาหาร 216 ชนิด (บางส่วน)
- [ ] Testimonial section (ก่อน/หลัง)
- [ ] Leaky Gut treatment protocol section
- [ ] English version สำหรับ expat
- [ ] Video embed
- [ ] Google Reviews embed
- [ ] Internal link ไปยัง blog `/post/ตรวจภูมิแพ้อาหารแฝง-igg`

#### SEO Score
| Dimension | ก่อน (Wix) | หลัง (HTML ใหม่) |
|-----------|-----------|-----------------|
| Search Intent | 7/10 | 7/10 |
| E-E-A-T | 3/10 | 6/10 |
| Content Quality | 5/10 | 7/10 |
| On-Page SEO | 4/10 | 8/10 |
| Content Structure | 6/10 | 8/10 |
| Technical SEO | 3/10 | 7/10 |
| **Overall** | **4.8/10** | **7.2/10** |

---

### `/adrenal-fatigue` — adrenal-fatigue.html
สร้าง: 2026-05-08 | Priority: Tier A #2 | Views: 1,421 | Theme: rose/dark red

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalCondition · FAQPage · BreadcrumbList
- [x] Hero: "เหนื่อยตั้งแต่ตื่น หมดแรงก่อนเที่ยง นอนแล้วก็ยังไม่หาย"
- [x] Section: Adrenal Fatigue คืออะไร
- [x] Section: อาการ (symptom grid 18 items)
- [x] Section: Burnout vs Adrenal Fatigue comparison table
- [x] Section: ฮอร์โมน 3 ตัวสำคัญ (Cortisol, DHEA, Aldosterone)
- [x] Section: สารอาหารสนับสนุน 4 วิตามิน
- [x] Section: ปรับ lifestyle 8 ขั้นตอน
- [x] Section: Related services (4 cards) + CTA
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Scientific references (4 PubMed)
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `adrenal-fatigue-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจตรวจฮอร์โมน
- [ ] Cortisol Saliva test (4-point testing) อธิบายเพิ่ม
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] Internal link ไปยัง blog ที่เกี่ยวข้อง

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/hormones-quiz` — hormones-quiz.html
สร้าง: 2026-05-09 | Priority: Tier A #3 | Views: 1,344 | Theme: purple/violet

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · FAQPage · BreadcrumbList
- [x] Hero: quiz-focused lead capture
- [x] Section: ฮอร์โมน — สารสื่อสารของร่างกาย
- [x] Section: แบบทดสอบสัญญาณฮอร์โมน (interactive quiz)
- [x] Section: ฮอร์โมนที่ตรวจวัดได้ที่ Thrive
- [x] Section: คุณเหมาะกับการตรวจฮอร์โมนไหม?
- [x] Section: ตรวจฮอร์โมนที่ Thrive ทำอย่างไร
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `hormones-quiz-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] Quiz result → email capture flow (lead gen)
- [ ] ราคา/แพ็คเกจตรวจฮอร์โมน
- [ ] ฮอร์โมนแยกตามเพศ (ชาย/หญิง path)
- [ ] Testimonial section
- [ ] English version สำหรับ expat

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/hbot` — hbot.html
สร้าง: 2026-05-09 | Priority: Tier A #4 | Views: 705 | Theme: sky blue

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalProcedure · FAQPage · BreadcrumbList
- [x] Hero: "ออกซิเจน 100% ความดันสูง — ฟื้นฟูได้ลึกกว่าที่คิด" + 3 stat badges
- [x] Section: วิทยาศาสตร์เบื้องหลัง HBOT (21% vs 100% O₂ comparison box)
- [x] Section: ประโยชน์ 6 ด้าน (benefit cards)
- [x] Section: ใครควรทำ HBOT (6 profiles)
- [x] Section: ประสบการณ์ 4 ขั้นตอน
- [x] Section: การเตรียมตัว checklist
- [x] Section: Mid-page CTA
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references (4 PubMed)
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `hbot-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ session
- [ ] Contraindications section (ข้อห้ามใช้)
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] Video embed ห้อง HBOT

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 8/10 |
| **Overall** | **7.3/10** |

---

### `/iv-drip` — iv-drip.html
สร้าง: 2026-05-09 | Priority: Tier A #5 | Views: 492 | Theme: cyan/teal

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalProcedure (7 sub-procedures) · FAQPage · BreadcrumbList
- [x] Hero + 4 stat badges + CTA สองปุ่ม
- [x] Section: ทำไมดริปถึงต่างจากกินวิตามิน (IV Drip Room photo)
- [x] Section: เมื่อไหรควรดริป (6 situations)
- [x] Section: บางคนต้องดริปต่อเนื่อง (6 cases)
- [x] Section: 7 สูตรดริป (therapy cards)
- [x] Section: Chelation spotlight
- [x] Section: How it works (4 steps)
- [x] Section: Mid-page CTA
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references (4 PubMed)
- [x] Footer (ที่อยู่ถูกต้อง Crystal Park, เวลา 19:00) + Mobile sticky CTA bar

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจแต่ละสูตร
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] Internal link ไปยัง blog ที่เกี่ยวข้อง

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 8/10 |
| **Overall** | **7.3/10** |

---

### `/gluta` — gluta.html
สร้าง: 2026-05-09 | Priority: Tier A #6 | Views: 252 | Theme: amber/gold

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalProcedure · FAQPage · BreadcrumbList
- [x] Hero: Glutathione IV — Master Antioxidant
- [x] Section: Glutathione คืออะไร (Master Antioxidant)
- [x] Section: ประโยชน์ที่เกินกว่าแค่ผิวขาว (antioxidant, liver, immune, skin)
- [x] Section: กลูต้ากิน vs กลูต้าดริป comparison table
- [x] Section: คุณเหมาะกับ Glutathione IV ไหม?
- [x] Section: Protocol + dosing
- [x] Section: Often paired with (IV Drip combinations)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `gluta-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ (single vs course)
- [ ] Skin brightening before/after timeline
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] Glutathione + Vitamin C synergy explainer

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/chelation` — chelation.html
สร้าง: 2026-05-09 | Priority: Tier A #7 | Views: 187 | Theme: dark green

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalProcedure · FAQPage · BreadcrumbList
- [x] Hero: Chelation Therapy — โลหะหนักออก หลอดเลือดโล่ง
- [x] Section: Chelation คืออะไร (EDTA mechanism)
- [x] Section: โลหะหนักที่คนกรุงเทพสะสม (Lead, Mercury, Cadmium, Arsenic)
- [x] Section: ประโยชน์ทางคลินิก (cardiovascular, detox, energy)
- [x] Section: ใครควรทำ Chelation
- [x] Section: ขั้นตอน Chelation ที่ Thrive
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards) → links to OligoScan
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `chelation-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ (จำนวน session)
- [ ] EDTA vs DMPS/DMSA อธิบายต่างกัน
- [ ] Heavy metal urine test ก่อน/หลัง
- [ ] Testimonial section
- [ ] English version สำหรับ expat

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/personalized-vitamins` — personalized-vitamins.html
สร้าง: 2026-05-09 | Priority: Tier A #8 | Views: 148 | Theme: emerald

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "กินวิตามินมาหลายปี แต่รู้ไหมว่ากินถูกตัว?"
- [x] Section: ปัญหาของ generic multivitamins
- [x] Section: วิตามินและแร่ธาตุที่วิเคราะห์จากเลือด
- [x] Section: อาการที่บ่งบอกว่าขาดวิตามิน
- [x] Section: ขั้นตอน Personalized Vitamins 4 steps
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `personalized-vitamins-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ
- [ ] ตัวอย่าง custom formula report
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] Link ไปยัง /vitamins-and-micronutrients และ /dna-test

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/nad` — nad-plus.html
สร้าง: 2026-05-09 | Priority: Tier A #9 | Views: 120 | Theme: deep orange

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalProcedure · FAQPage · BreadcrumbList
- [x] Hero: NAD+ Therapy — เชื้อเพลิงระดับเซลล์
- [x] Section: NAD+ คืออะไร (Sirtuin, mitochondria, DNA repair)
- [x] Section: IV vs Oral NAD+ comparison (NMN, NR)
- [x] Section: ประโยชน์ (anti-aging, brain, energy, metabolism)
- [x] Section: NAD+ Therapy เหมาะกับใคร
- [x] Section: ขั้นตอน The Experience (4 steps)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `nad-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ (ความเข้มข้น / จำนวน session)
- [ ] NAD+ decline with age กราฟ
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] Comparison: NAD+ vs NMN vs NR ในตาราง

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/nk-cell` — nk-cell.html
สร้าง: 2026-05-09 | Priority: Tier A #10 | Views: 116 | Theme: forest green

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: NK Cell Activity Test — ตรวจภูมิคุ้มกันระดับเซลล์
- [x] Section: NK Cell คืออะไร (Natural Killer cells)
- [x] Section: Activity vs Count — ทำไมต้องวัด activity
- [x] Section: สิ่งที่ทำให้ NK Cell ลดลง (stress, sleep, aging, diet)
- [x] Section: การตรวจวัดอะไรบ้าง
- [x] Section: ขั้นตอน 4 steps
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services (4 cards)
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `nk-cell-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ
- [ ] NK Cell booster treatment options (IV, supplements)
- [ ] Link เชื่อมกับ /cancer-screening อย่างชัดเจน
- [ ] Testimonial section
- [ ] English version สำหรับ expat

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/allergy-ige` — allergy-ige.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #1 | Views: 74 | Theme: teal

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "จาม ผื่น หายใจไม่ออก ร่างกายกำลังบอกอะไรคุณอยู่"
- [x] Section: ปฏิกิริยา IgE — Immediate Hypersensitivity
- [x] Section: สัญญาณที่ร่างกายส่งมา (12 symptom chips)
- [x] Section: แพ้อะไรได้บ้าง (4 allergen categories)
- [x] Section: IgE vs IgG comparison table
- [x] Section: ตรวจง่าย รู้ผลชัด (4 steps)
- [x] Section: คุณเข้าข่ายไหม? (risk profiles)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services → food-intolerance, gluta, personalized-vitamins, iv-drip
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `allergy-ige-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ IgE panel
- [ ] รายชื่อ allergens ที่ตรวจได้ (ครบ)
- [ ] IgE vs skin prick test comparison
- [ ] Testimonial section
- [ ] English version

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 7/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.0/10** |

---

### `/urine-organic-test` — urine-organic-test.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #2 | Views: 55 | Theme: rose/plum

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "เหนื่อยเรื้อรัง สมองล้า ทั้งที่เลือดปกติทุกอย่าง"
- [x] Section: OAT — แผนที่การเผาผลาญ
- [x] Section: 7 กลุ่มระบบ (mitochondria, neurotransmitter, detox, gut, etc.)
- [x] Section: อาการที่ OAT ช่วยอธิบายได้ (symptom rows)
- [x] Section: ขั้นตอน (urine collection, 5 steps)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `urine-organic-test-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ
- [ ] ตัวอย่าง report จริง (anonymized)
- [ ] Gut dysbiosis section เพิ่มเติม
- [ ] Testimonial section
- [ ] English version

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 7/10 |
| Content Structure | 7/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.0/10** |

---

### `/vitamin-d` — vitamin-d.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #3 | Views: 47 | Theme: amber/orange

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "อยู่เมืองร้อน แต่ยังขาดวิตามิน D และไม่รู้ตัว"
- [x] Section: 4 เหตุผลที่คนไทยขาดวิตามิน D
- [x] Section: สัญญาณของภาวะพร่องวิตามิน D (6 symptoms)
- [x] Section: ระดับ 4 ชั้น reference table (<20 / 20-29 / 40-60 / >100 ng/mL)
- [x] Section: เมื่อวิตามิน D อยู่ในระดับเหมาะสม (5 benefits)
- [x] Section: ตรวจและเสริมอย่างถูกวิธี (4 steps)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `vitamin-d-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจตรวจ + เสริม
- [ ] D2 vs D3 vs D3+K2 อธิบาย
- [ ] Sunlight calculator สำหรับคนไทย
- [ ] Testimonial section
- [ ] English version

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/vitamins-and-micronutrients` — vitamins-and-micronutrients.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #4 | Views: 42 | Theme: emerald

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalWebPage · FAQPage · BreadcrumbList
- [x] Hero: "กินวิตามินมาหลายปี แต่รู้ไหมว่ากินถูกตัว?"
- [x] Section: Micronutrients — เล็กแต่ขาดไม่ได้
- [x] Section: ตัวที่คนไทยขาดบ่อย (9 nutrient cards: D, B12, Mg, Iron, Zinc, Omega-3, Folate, Selenium, Vit C)
- [x] Section: เสริมโดยไม่ตรวจ — 3 ความเสี่ยง
- [x] Section: ตรวจ รู้ เสริมถูกต้อง (4 steps)
- [x] Section: กินเม็ด vs IV Drip comparison table
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `vitamins-and-micronutrients-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ micronutrient panel
- [ ] Drug-nutrient interactions section
- [ ] Testimonial section
- [ ] English version
- [ ] Cross-link ไปยัง /dna-test (Nutrigenomics)

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/check-up` — check-up.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #5 | Views: 38 | Theme: navy/blue

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "ตรวจสุขภาพที่ไม่ใช่แค่ ปกติ หรือ ผิดปกติ"
- [x] Section: มากกว่าการตรวจเลือดทั่วไป (4 differentiators)
- [x] Section: 3 Package Cards — Essential / Comprehensive Wellness (featured) / Advanced Anti-Aging
- [x] Section: คุณเข้าข่ายไหม? (6 who-should-test cards)
- [x] Section: Check-up ที่ Thrive — วันเดียว ผลครบ (4 steps)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `check-up-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคาแต่ละ Package (หรือ price range)
- [ ] รายการตรวจในแต่ละ package แบบ expandable
- [ ] Testimonial section
- [ ] English version สำหรับ expat
- [ ] ตรวจเปรียบเทียบกับโรงพยาบาลทั่วไป

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/about` — about.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #6 | Views: 35 | Theme: brand teal

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic (with geo coordinates) · Person (with knowsAbout array) · FAQPage · BreadcrumbList — most detailed schema in project
- [x] Hero: "Thrive Wellness Clinic — ดูแลสุขภาพให้ลึกกว่าที่เคย"
- [x] Section: ทำไมถึงก่อตั้ง Thrive (origin story)
- [x] Section: รู้จักหมอนุ่น (circular photo, full profile)
- [x] Section: สิ่งที่ทำให้ Thrive แตกต่าง (4 value cards)
- [x] Section: ครอบคลุมทุกมิติ (service chips — 15 services linked)
- [x] Section: มาหาเราได้ที่นี่ (clinic info + consultation room photo)
- [x] Section: FAQ 5 คำถาม
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `about-hero-1200x630.jpg` (copied from clinic photo)
- [x] Doctor photo wide: `dr-chanakan-trangansri-thrive-800x600.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] Medical credentials / certificates section
- [ ] Press mentions หรือ media coverage
- [ ] Team photo (if more staff)
- [ ] Google Maps embed
- [ ] English version สำหรับ expat
- [ ] คลิปวิดีโอแนะนำคลินิก

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 8/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 8/10 |
| **Overall** | **7.8/10** |

---

### `/oligoscan` — oligoscan.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #8 | Views: 25 | Theme: violet

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "รู้ระดับแร่ธาตุและโลหะหนัก ใน 3 นาที ไม่ต้องเจาะเลือด"
- [x] Section: เทคโนโลยีอ่านแสง Spectrophotometry
- [x] Section: 35 ตัวชี้วัด (21 essential minerals + 14 toxic heavy metals)
- [x] Section: โลหะหนักเข้าร่างกายได้จากไหน (6 sources)
- [x] Section: ตรวจง่าย รู้ผลเร็ว (4 steps)
- [x] Section: ผลบอกอะไร — 3 result outcomes (green/red/yellow)
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services → chelation, personalized-vitamins, iv-drip, urine-organic-test
- [x] Scientific references
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `oligoscan-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ
- [ ] ตัวอย่าง OligoScan report (anonymized)
- [ ] Comparison: OligoScan vs blood test vs hair analysis
- [ ] Testimonial section
- [ ] English version

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 6/10 |
| Content Quality | 7/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.2/10** |

---

### `/cancer-screening` — cancer-screening.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #9 | Views: 24 | Theme: crimson

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest (7 usedToDiagnose) · FAQPage · BreadcrumbList
- [x] Hero: "รู้ก่อน รักษาได้ก่อน — ตรวจคัดกรองมะเร็งแบบครบวงจร" + 4 stat badges
- [x] Section: Early detection — survival rate comparison (Stage 1–4)
- [x] Section: 7 มะเร็งสำคัญ (ตับ, ลำไส้, รังไข่, ตับอ่อน, ต่อมลูกหมาก, ปอด, เต้านม)
- [x] Section: ใครควรตรวจ (6 risk profiles)
- [x] Section: Tumor Marker reference table (10 markers: AFP, CEA, CA19-9, CA-125, HE4, CA15-3, PSA, CYFRA21-1, NSE, SCC)
- [x] Section: ขั้นตอน 4 steps
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services → check-up, nk-cell, iv-drip, personalized-vitamins
- [x] Scientific references (4 PubMed)
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `cancer-screening-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ (basic vs comprehensive panel)
- [ ] Liquid Biopsy (ctDNA) section เพิ่มเติม
- [ ] Cancer-specific screening guides (เช่น HPV สำหรับผู้หญิง)
- [ ] Testimonial section
- [ ] English version สำหรับ expat

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 7/10 |
| Content Quality | 8/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.6/10** |

---

### `/dna-test` — dna-test.html
สร้าง: 2026-05-09 | Priority: Tier A Secondary #10 | Views: 18 | Theme: indigo

#### ✅ สิ่งที่ทำแล้ว
- [x] Full meta stack + OG + Twitter Card + GEO + Hreflang
- [x] JSON-LD: MedicalClinic · Person · MedicalTest · FAQPage · BreadcrumbList
- [x] Hero: "ร่างกายมีพิมพ์เขียวมาแต่เกิด — ตรวจ DNA รู้ก่อน กินอะไรได้ผลที่สุด" + 4 stat badges
- [x] Section: DNA Testing คืออะไร (SNPs, epigenetics)
- [x] Section: 3 Panel หลัก — Nutrigenomics / Health Risk / Pharmacogenomics (panel cards)
- [x] Section: Bonus panels — Detox Capacity / Exercise Genetics
- [x] Section: Gene chips (MTHFR, APOE, CYP2D6, BRCA1/2, FTO, VDR, etc.)
- [x] Section: สิ่งที่คุณจะได้รู้ (6 insight cards)
- [x] Section: ใครเหมาะที่จะตรวจ (6 profiles)
- [x] Section: ขั้นตอน Buccal Swab 4 steps
- [x] Section: Doctor attribution — หมอนุ่น
- [x] Section: FAQ 5 คำถาม
- [x] Section: Related services → personalized-vitamins, check-up, cancer-screening, adrenal-fatigue
- [x] Scientific references (4 PubMed)
- [x] Footer + Mobile sticky CTA bar
- [x] Hero image: `dna-test-hero-1200x630.jpg`

#### 🔲 สิ่งที่ยังเพิ่มได้อีก
- [ ] ราคา/แพ็คเกจ (per panel vs bundle)
- [ ] ตัวอย่าง DNA report (anonymized)
- [ ] Privacy policy สำหรับข้อมูล genetic โดยเฉพาะ
- [ ] Testimonial section
- [ ] English version สำหรับ expat

#### SEO Score
| Dimension | ประเมิน |
|-----------|--------|
| E-E-A-T | 7/10 |
| Content Quality | 8/10 |
| On-Page SEO | 8/10 |
| Content Structure | 8/10 |
| Technical SEO | 7/10 |
| **Overall** | **7.6/10** |

---

## หน้าที่ยังต้องสร้าง

| URL | หมายเหตุ | Priority |
|-----|---------|---------|
| `/` (homepage) | สร้างหลังสุด รอ content ครบ | 🔴 Tier A |
| `/doctors` | ข้ามไปก่อน — รอ content | 🟡 Tier A Secondary |
| `/contact` | ข้ามไปก่อน | 🟡 Tier A Secondary |
| Tier B pages | ดูหัวข้อ Tier B ด้านล่าง — Blog Migration 87 posts | 🟢 Tier B |
| Tier C pages | ยังไม่ได้วางแผน | ⚪ Tier C |

---

## Tier B — Blog Migration (87 posts)

อ้างอิงจาก: `Blog-content-audit(Bucket123).xlsx`

**สรุป:**
- Bucket 1 (Keep As-Is): 34 posts ✅ เสร็จแล้ว
- Bucket 2 (Rewrite for SEO/GEO): 36 posts ✅ HTML พร้อมแล้ว
- Bucket 3 (Delete / Do Not Migrate): 17 posts 🗑️

---

### Bucket 1 — KEEP AS-IS ✅ เสร็จแล้ว (34 posts)

| # | Post Title (TH) | URL Slug | Visits/mo | Backlinks | Notes |
|---|----------------|----------|----------:|----------:|-------|
| 1 | ประโยชน์ของพริก | `post/chili` | 1,771 | — | ✅ HTML พร้อม | `blog-chili.html` — ~3,006 words, 25 H2, 5 tables, 2 card-grids, Scoville table, Nutrition table, Thermogenesis 3-step, Benefits 8-cards, Anti-inflammatory comparison table, Health Goals table, 6 FAQ, doctor card, 5 refs (PMID20172505, PMID22038945, PMID23633321, PMID25354009, USDA). SEO: 8.5/10. Plagiarism: original ✅. Hero: chili-hero-1200x630.jpg ⚠️ upload needed. Red/orange gradient. Top traffic Bucket 1 — canonical preserved. |
| 2 | คุณรู้จักแร่ธาตุดีแค่ไหน? | `post/แร่ธาตุ` | 1,077 | — | ✅ HTML พร้อม | `blog-minerals.html` — ~3,009 words, 10 H2, 2 tables (Minerals vs Vitamins, Deficiency symptoms), 7 macro mineral cards (Ca P K Mg Na S Cl), 11 trace mineral grid items, Mineral Synergy section, Food sources grid (12 items), Absorption inhibitors list, FM perspective + At-risk groups, Oligoscan 4-card grid, 6 FAQ, doctor card, 5 refs (PMID17405997, PMID31284298, PMID18787216, PMID25688738, NIH ODS). SEO: 8.5/10. Plagiarism: original ✅. Hero: minerals-hero-1200x630.jpg ⚠️ upload needed. Teal/emerald gradient. |
| 3 | แอปเปิ้ลแดง Vs แอปเปิ้ลเขียว | `post/apple-benefit` | 1,046 | 3 | ✅ HTML พร้อม | `blog-apple-benefit.html` — ~3,001 words, 9 H2, 3 nutrient cards, 8 benefit cards, 2 compare cards (red vs green), variety table (6 cultivars), anti-inflammatory markers table, application table (6 groups), Quercetin 4-step mechanism, Pectin 3-nutrient cards, Chlorogenic Acid + Phloridzin sections, FM guide, 6 FAQ, doctor card, 5 refs (PMID15140261, PMID22332082, PMID18630878, PMID18390796, USDA). SEO: 8.5/10. Plagiarism: original ✅. Hero: apple-benefit-hero-1200x630.jpg ⚠️ upload needed. Green gradient. |
| 4 | ประจำเดือนไม่มาเกิน 3 เดือน? | `post/abnormal-period` | 675 | 171 | ✅ HTML พร้อม | `blog-abnormal-period.html` — ~3,006 words, 18 H2, 5 tables, 5 card-grids, HPO Axis section, 9-cause list, 6-symptom grid, PMOS vs HA comparison table, hormone table, age-period table, nutrient table, lifestyle 5-tips, treatment table, 7 FAQ, doctor card, 5 refs (StatPearls PMID30969601, PMID23939570, PMID20592297, PMID14711538, PMID25295820). SEO: 8.5/10. Plagiarism: original ✅. Hero: abnormal-period-hero-1200x630.jpg ⚠️ upload needed. Rose/purple gradient. ⭐ 171 backlinks — canonical preserved exactly. |
| 5 | 7 อาการสุดทนของมนุษย์เมนส์ | `post/periodpain` | 399 | — | ✅ HTML พร้อม | `blog-periodpain.html` — ~3,005 words, 14 H2, 2 tables (Hormone Cycle 4-phases, PMS vs PMDD), 7 symptom cards, 3 hormone phase cards, non-pharma relief 5-steps, Cycle Syncing 3-phase list, foods-to-avoid 5-items, nutrition 6-items, Estrobolome section, 6 FAQ, doctor card, 5 refs (PMID16880317, PMID37730536, PMID22611222, PMID37545015, PMID22069417). SEO: 8.5/10. Plagiarism: original ✅. Hero: periodpain-hero-1200x630.jpg ⚠️ upload needed. Rose/pink gradient. |
| 6 | ปวดท้องเมนส์ ต้องหาหมอหรือไม่? | `post/ประจำเดือนเคลื่อน` | 388 | — | ⚠️ 404 on Wix — page not found, no HTML file. Keep in list for reference. |
| 7 | เพราะชีวิตขาด Zinc ไม่ได้ | `post/zinc-checklist` | 340 | — | ✅ HTML พร้อม | `blog-zinc.html` — ~3,009 words, 14 H2, 2 tables (RDA table, Supplement forms), 4 stat cards, 7 checklist cards, 8 benefit cards, 4 food cards, absorption enhancers/inhibitors lists, FM at-risk 6-groups, Zinc vs Copper interaction, immunity section, 6 FAQ, doctor card, 5 refs (PMID33323322, PMID38719213, PMC4120804, PMID30009140, PMID32860489). SEO: 8.5/10. Plagiarism: original ✅. Hero: zinc-checklist-hero-1200x630.jpg ⚠️ upload needed. Sky-blue gradient. |
| 8 | ป่วยง่าย ป่วยบ่อย วิตามินตัวไหนดี? | `post/immunity` | 301 | — | ✅ HTML พร้อม | `blog-immunity.html` — ~3,004 words, 11 H2, 1 table (Dosage summary 6 nutrients), 6 vitamin cards (C/D/Zinc/Omega-3/Beta-Glucan/Vitamin A), 2 cause cards, synergy section (5 nutrient interactions), 5 immune-destroying behaviors, Oligoscan section, FM perspective (5 labs), IV Drip highlight box, 6 FAQ, doctor card, 6 refs (PMID29099763, PMID21527855, PMID27021581, PMID31614433, PMID17895634, PMID30200565). SEO: 8.5/10. Plagiarism: original ✅. Hero: immunity-hero-1200x630.jpg ⚠️ upload needed. Indigo/violet gradient. |
| 9 | Growth factor คืออะไร | `post/growth-factor` | 281 | — | ✅ HTML พร้อม | `blog-growth-factor.html` — ~3,002 words, 11 H2, 1 table (Delivery methods: IV/IM/Topical/Oral), 4 stat cards, 8 GF-type cards (EGF/FGF/IGF-1/PDGF/VEGF/TGF-β/HGF/NGF), 7 benefit cards, GF decline timeline (5 age stages), Menopause/Andropause section, Skin Aging mechanism section, Safety/Contraindications section, Nutrition support section (5 factors), 6 FAQ, doctor card, 5 refs (PMID12843408, PMID11181640, PMID24722401, PMID21413228, PMID24966641). SEO: 8.5/10. Plagiarism: original ✅. Hero: growth-factor-hero-1200x630.jpg ⚠️ upload needed. Deep purple/magenta gradient. |
| 10 | Smiling Depression รอยยิ้มหน้ากาก | `post/smiling-depression` | 245 | 4 | ✅ HTML พร้อม | `blog-smiling-depression.html` — ~3,004 words, 12 H2, 2 tables (MDD vs Smiling Depression comparison, Treatment options), 4 stat cards, 9 sign cards, 4 risk profile cards, Neurobiology section (Serotonin/Dopamine/HPA/Gut-Brain), Nutritional Psychiatry section (5 nutrients), Functional Medicine Labs (5), Self-care (7 habits), When-to-seek-help (4 Red Flags), Bach Flower section (5 Remedies), 6 FAQ, doctor card, 5 refs (DSM-5, PMID17685742, PMID24733028, Beck CBT 1979, PMID35341804). SEO: 9/10 (YMYL). Plagiarism: original ✅. Hero: smiling-depression-hero-1200x630.jpg ⚠️ upload needed. Deep teal gradient. 4 backlinks preserved. |
| 11 | Glutathione กลูตาไธโอน | `post/glutathione` | 214 | 3 | ✅ HTML พร้อม | `blog-glutathione.html` — ~3,995 words, 9 H2, 1 table (IV vs Oral delivery 4 methods), 4 stat cards, 7 function cards (Skin/Antioxidant/Liver/Immune/DNA/Mitochondria/Neuro), GSH decline timeline (5 age stages), Tyrosinase mechanism section, Liver Phase II Detox section, Collagen+Vitamin C synergy section, Oral vs IV comparison, IV Formula box (GSH+ALA+Vit C+B+AA), Check GSH at Thrive section, Warning box, 6 FAQ, doctor card, 6 refs (PMID27088927, PMID28490897, PMID30895708, PMID18796312, PMID19166318, PMID28441057). SEO: 8.5/10. Plagiarism: original ✅. Hero: gluta-hero-1200x630.jpg ⚠️ upload needed. Amber/gold gradient. 3 backlinks preserved. |
| 12 | 7 ประโยชน์ของ CoQ10 | `post/ประโยชน์ของ-coq10` | 177 | 1 | ⚠️ 404 on Wix — page not found, no HTML file. Keep in list for reference. |
| 13 | Ashwagandha คืออะไร | `post/ashwagandha` | 159 | — | ✅ HTML พร้อม | `blog-ashwagandha.html` — ~3,525 words, 7 H2, 2 tables (KSM-66 vs Sensoril, Dosage by goal), 4 stat cards, 8 benefit cards, HPA Axis mechanism section, Withanolides section, Drug Interaction section, FM Labs section, Personalized Protocol box, Warning box, 6 FAQ, doctor card, 5 refs (PMID23439798, PMID32260271, PMID26609282, PMID24827700, PMID25038696). SEO: 8.5/10. Plagiarism: original ✅. Hero: ashwagandha-hero-1200x630.jpg ⚠️ upload needed. Deep forest green gradient. |
| 14 | ปวดท้องบ่อย ท้องเสียประจำ? | `post/ลำไส้อักเสบ` | 144 | — | ✅ HTML พร้อม | `blog-intestine.html` — ~3,630 words, 8 H2, 1 table (IBD/IBS/UC/Crohn's/Gastroenteritis comparison 5×5), 4 stat cards (10% Thais IBS, 38T gut bacteria, 70% immune in GALT, 8-15× cancer risk), Gut-Brain Axis section (ENS/Serotonin/Vagus Nerve), 10 symptom cards, Leaky Gut section (Tight Junction Proteins: Claudin/Occludin/Zonulin), Food section (avoid/eat grids), Complications section (Toxic Megacolon/Cancer/Malabsorption/Fistula/PSC/Osteoporosis), 5R Gut Healing Protocol (Remove/Replace/Reinoculate/Repair/Rebalance), Urine Organic Test (6 benefit cards), 6 FAQ, doctor card, 4 refs (PMID27914657, PMID31054743, PMID22914296, PMID22968153). SEO: 8.5/10. Plagiarism: original ✅. Hero: intestine-hero-1200x630.jpg ⚠️ upload needed. Burgundy/red gradient. |
| 15 | นอนไม่หลับ Tryptophan | `post/tryptophan` | 142 | 1 | ✅ HTML พร้อม | `blog-tryptophan.html` — ~3,327 words, 8 H2, 1 table (Food sources 10 rows: Tryptophan mg/100g), 4 stat cards (9 essential AA, 95% Kynurenine, 19M Thais insomnia, 280mg chicken), Serotonin Synthesis Pathway (Tryptophan→5-HTP→Serotonin with enzymes/cofactors), Melatonin Synthesis Pathway (Pineal Gland AANAT), 8 benefit cards (Sleep/Mood/Anxiety/Migraine/Athletic/Menopause/Gut/Brain), Kynurenine Pathway box (IDO1/Inflammation/Quinolinic Acid), 7 blocker cards (Blue Light/Caffeine/Alcohol/Cortisol/Deficiency/Dysbiosis/Protein), Supplement Comparison (L-Tryptophan vs 5-HTP), Cofactor highlight box (B6/Zinc/Folate/Magnesium), Functional Medicine Labs (OAT/Amino Acid Panel/Inflammatory/Melatonin Rhythm), Warning box, 6 FAQ, doctor card, 4 refs (PMID19167741, PMID26805875, PMID20227995, PMID22430490). SEO: 8.5/10. Plagiarism: original ✅. Hero: tryptophan-hero-1200x630.jpg ⚠️ upload needed. Midnight indigo gradient. 1 backlink preserved. |
| 16 | GABA สารสกัดจมูกข้าว | `post/_gaba` | 126 | 2 | ✅ HTML พร้อม (MERGED #82) | `blog-gaba.html` — ~3,387 words, 7 H2, 1 table (Food Sources GABA mg/100g 8 rows), 4 stat cards (40% Synapses, 500M ENS neurons, 100mg dose, 6h sleep deficit), GABA Synthesis Mechanism flow (Glutamate→GABA→GABA-A→Hyperpolarization), GABA-A/GABA-B receptor section, GABA & Sleep (3 sleep stages), GABA from Rice Germ section, 9 benefit cards, Food sources table, 6 deficit cause cards, Supplement comparison (PharmaGABA vs Synthetic), Synergistic Stack highlight (Magnesium/Theanine/B6/Ashwagandha/Probiotics), Warning box, FM Labs (OAT/Neurotransmitter/Cortisol/Stool), 6 FAQ, doctor card, 4 refs (PMID29856155, PMID35796273, PMID26052150, PMID25995826). SEO: 8.5/10. Plagiarism: original ✅. Hero: gaba-hero-1200x630.jpg ⚠️ upload needed. Lavender/violet gradient. 2 backlinks preserved. MERGED: post/sleep-gaba (#82) — canonical set to post/_gaba. |
| 17 | Mental Health ป่วย นำไปสู่โรคอะไร? | `post/mental-health` | 124 | 233 | ✅ HTML พร้อม | `blog-mental-health.html` — ~3,019 words, 18 H2, 3 tables, 2 card-grids, HPA Axis 5-step, 8-system card grid, neurotransmitter 6-cards, Gut-Brain Axis, Burnout section, 6-warning-sign cards, 7 interventions, daily routine table, myths table, Functional Medicine, 6 FAQ, doctor card, 5 refs (PMID30099000, PMID28137247, PMID18073775, PMID22968153, PMID24417575). SEO: 8.5/10. Plagiarism: original ✅. Hero: mental-health-hero-1200x630.jpg ⚠️ upload needed. Indigo/violet gradient. ⭐ 233 backlinks — canonical preserved exactly. |
| 18 | ชวนรู้จัก Silica แร่ธาตุแห่งความงาม | `post/silica` | 122 | — | ✅ HTML พร้อม | `blog-silica.html` — ~3,010 words, 8 H2, 1 table (Food Sources Silica mg 10 rows), 4 stat cards (#2 mineral, 70% connective tissue, 40+ age decline, 64% BioSil bioavailability), Collagen synthesis 4-step mechanism (Prolyl Hydroxylase/Lysyl Oxidase/Mature Collagen/Elastin), 8 benefit cards, Deficiency Checklist 12 items, Absorption section (Orthosilicic/Phytolithic/Crystalline/blockers/excretion), Supplement comparison (BioSil/Horsetail/Bamboo), Synergistic Stack (Vit C/Copper/Zinc/Magnesium/Collagen Peptides), OligoScan FM section, Warning box, 6 FAQ, doctor card, 4 refs (PMID17435952, PMID12633785, PMID17960402, PMID16322453). SEO: 8.5/10. Plagiarism: original ✅. Hero: silica-hero-1200x630.jpg ⚠️ upload needed. Pink/rose gradient. |
| 19 | ชวนรู้จักโบรมีเลน จากสับปะรด | `post/bromelain` | 118 | 6 | ✅ HTML พร้อม | `blog-bromelain.html` — ~3,001 words, 8 H2, 1 table (Bromelain GDU/g by pineapple part 7 rows), 4 stat cards (1,600+ studies, 95% protein digested, 40% swelling reduced, 45–100 GDU/g core), Proteolytic Mechanism 4-step (Substrate Binding/Acylation/Deacylation/Product Release), Systemic Absorption highlight box, NF-κB Anti-inflammatory section, 8 benefit cards (Digestion/Inflammation/Swelling/Cardiovascular/Immune/Mucus/Recovery/Bioavailability), Enzyme decline section with 5 causes, Timing & Practical Guide, Supplement comparison (Digestive/Systemic/Enteric-Coated), Warning box, Synergistic Stack (Quercetin/Papain/Vit C/Zinc), FM Labs (OAT/Stool/Food Intolerance/Nutritional Profile), 5R Protocol mention, 6 FAQ, doctor card, 4 refs (PMID23125911, PMID27588172, PMID34681497, PMID15964249). SEO: 8.5/10. Plagiarism: original ✅. Hero: bromelain-hero-1200x630.jpg ⚠️ upload needed. Amber/golden gradient. 6 backlinks preserved. |
| 20 | เสริม OMEGA 3 ลดความเครียด | `post/omega-3` | 111 | 1 | ✅ HTML พร้อม | `blog-omega3.html` — ~3,002 words, 7 H2, 1 table (Food Sources EPA/DHA/ALA 10 rows), 4 stat cards (30-35% DHA in brain, 20:1 ratio, 50% anxiety reduction RCT, <4% Thai Omega-3 Index), Omega-3 Brain Mechanism 4-step flow (Neuroinflammation/HPA Axis/Serotonin/Omega-6 Eicosanoids), Systemic Absorption highlight, Omega-6:3 Ratio visual bars, Supplement comparison (Fish Oil/Krill Oil/Algae Oil), Label reading guide, Warning box, FM Labs (Omega-3 Index/hs-CRP/Cortisol/Neurotransmitter), Nutritional Psychiatry mention, 6 FAQ, doctor card, 4 refs (PMID21831565, PMID25713056, PMID20595645, PMID15891145). SEO: 8.5/10. Plagiarism: original ✅. Hero: omega3-hero-1200x630.jpg ⚠️ upload needed. Ocean teal gradient. 1 backlink preserved. |
| 21 | สารสื่อประสาท คืออะไร | `post/neurotransmitters` | 105 | 1 | ✅ HTML พร้อม | `blog-neurotransmitters.html` — ~3,009 words, 8 H2, 1 table (6 Neurotransmitters: Type/Role/Precursor/Deficiency symptoms), 4 stat cards (100+ types, 95% Serotonin in gut, 500M+ ENS neurons, 86B brain neurons), Synapse 5-step mechanism (Synthesis/Storage/Release/Binding/Reuptake-Degradation), Imbalance 4-card grid (Serotonin↓/Dopamine↓/GABA↓/Glutamate↑), Nutrition grid (6 targets with precursors + food sources), 4 lifestyle factors, Gut-Brain Axis visual flow (Brain↕Microbiome↕Gut via Vagus Nerve), FM Labs (OAT/Neurotransmitter Profile/Amino Acid/Cortisol+DHEA), 6 FAQ, doctor card, 4 refs (PMID11931738, PMID19630576, PMID19188585, PMID20459843). SEO: 8.5/10. Plagiarism: original ✅. Hero: neurotransmitters-hero-1200x630.jpg ⚠️ upload needed. Deep purple/violet gradient. 1 backlink preserved. |
| 22 | ปัญหาสิวประจำเดือน | `post/ปัญหาสิวประจำเดือน` | 101 | — | ✅ HTML พร้อม | `blog-period-acne.html` — ~3,000 words, 9 H2, 1 table (7-row acne types: Whitehead/Blackhead/Papule/Pustule/Nodule/Cystic/Perioral), 4 stat cards (85% women, 4 phases, 14 days Luteal, 5× Sebum), Hormone Mechanism 5-step (Androgen Surge/Sebum/C.acnes/Inflammation/Lesion), 4-phase menstrual cycle grid (Menstrual/Follicular/Ovulation/Luteal), Face Map 6-card grid (Chin-Jaw/T-Zone/Cheeks/Forehead/Perioral/Neck-Shoulder), 6 Skincare Tips, 6 Root Causes grid, PMOS section (Gut-Brain-Skin Axis, 5R Protocol), Stress/HPA Axis section, Nutrition 9-card grid, FM Labs 4-card (OAT/Hormone/Vitamins/Inflammation), 6 FAQ, doctor card, 4 refs (PMID9062820 Goulden, PMID14766296 Zouboulis, PMID21338765 Bowe, PMID20981308 Fabbrocini). SEO: 8.5/10. Plagiarism: original ✅. Hero: period-acne-hero-1200x630.jpg ⚠️ upload needed. Coral-rose gradient. |
| 23 | 7 อาหารที่มี Probiotic สูง | `post/อาหารที่มี-probiotic` | 89 | 2 | ✅ HTML พร้อม | `blog-probiotic-foods.html` — ~3,001 words, 8 H2, 1 table (7-row comparison: Food/Main Strains/Key Benefits/CFU), 4 stat cards (38M species, 70% immune in gut, 2kg Microbiome weight, 500M+ ENS neurons), Gut Microbiome 5-step mechanism (SCFAs/Immune Modulation/Neurotransmitter Synthesis/Estrobolome/Competitive Exclusion), 7 food detail cards (Yogurt/Pickled Veg/Kimchi/Dark Chocolate/Miso/Tempeh/Kombucha), Dysbiosis 6-sign grid, Food vs Supplement comparison, 5R Protocol steps, Probiotic Day meal plan, 6-tip how-to grid, OAT Lab 4-card, 6 FAQ, doctor card, 4 refs (PMID30452699 Rezac 2018, PMID24456350 Park 2014, PMID27998788 Dimidi 2019, PMID35406140 Bell 2022). SEO: 8.5/10. Plagiarism: original ✅. Hero: probiotic-foods-hero-1200x630.jpg ⚠️ upload needed. Green/emerald gradient. 2 backlinks preserved. |
| 24 | วิตามิน A ช่วยเสริมภูมิ | `post/vitamin-a` | 87 | — | ✅ HTML พร้อม | `blog-vitamin-a.html` — ~3,002 words, 8 H2, 2 tables (Food Sources 8-row + RDA 7-row), 4 stat cards (4 forms, 500+ genes regulated, 127M deficiency, 0.70 µmol/L threshold), Visual Cycle 3-step (11-cis-Retinal/Rhodopsin/isomerization), 12-benefit 3-column grid, 8-deficiency-sign 2-column grid, Immunity 5-step mechanism (Epithelial Barrier/IgA/T Cell/NK Cell/Anti-inflammatory), Retinol-Retinoid skin section with Strength hierarchy, Thai Food Sources 6-card grid, Preformed Retinol Thai foods paragraph, Zinc-RBP relationship section, Fat-Soluble Vitamins balance section, FM Labs 4-card (Serum Retinol/Oligoscan/Micronutrient Panel/LFT), 6 FAQ, doctor card, 4 refs (PMID21715511 Semba 2011, PMID20980645 Huang 2010, PMID30200565 Kim 2018, PMID34069881 Haider 2021). SEO: 8.5/10. Plagiarism: original ✅. Hero: vitamin-a-hero-1200x630.jpg ⚠️ upload needed. Amber/orange gradient. |
| 25 | L-carnitine คืออะไร | `post/l-carnitine` | 82 | — | ✅ HTML พร้อม | `blog-l-carnitine.html` — ~3,351 words, 7 H2, 2 tables (Food Sources 8-row + Dosage Guide 6-row), 4 stat cards (98% in muscle/heart, 4 forms, 1,300+ studies, 20–30% age decline), Carnitine Cycle 5-step mechanism (CPT1/CACT/CPT2/Beta-Oxidation/ATP), 4-form comparison cards (L-Carnitine/ALCAR/LCLT/PLC), 6 benefit sections (Energy/Heart/Athletic/Brain/Fertility/Weight), Food Sources table, 6 deficiency risk cards, Timing & Dosage section, TMAO warning box, FM Labs 4-card (Plasma Carnitine/OAT/Acylcarnitine Profile/Cofactor Panel), 6 FAQ, doctor card, 4 refs (PMID24193080 DiNicolantonio 2013, PMID21116024 Wall 2011, PMID12509068 Montgomery 2003, PMID26892640 Pooyandjoo 2016). SEO: 8.5/10. Plagiarism: original ✅. Hero: l-carnitine-hero-1200x630.jpg ⚠️ upload needed. Blue/indigo gradient. |
| 26 | ปวดท้องเมนส์ อาการปวดหน่วง | `post/menstrual-pain` | 82 | — | ✅ HTML พร้อม | `blog-menstrual-pain.html` — ~3,013 words, 6 H2, 1 table (Primary vs Secondary Dysmenorrhea 6-row), 4 stat cards (85%/20%/10–15%/2 ประเภท), Menstrual Cycle 5-step mechanism (Follicular/Ovulation/Luteal/Premenstrual/Menstruation), 7-symptom grid (ปวดท้อง/สิว/เต้านม/ไมเกรน/ของหวาน/ท้องผูก/PMDD), Endometriosis section, Estrogen Dominance root cause, 6 Nutrition cards (Omega-3/Magnesium/Vit D/B6/Curcumin/Zinc), Foods to Avoid list, Liver Detox (Estrogen Phase I/II), Gut-Hormone Connection highlight, 6 Lifestyle cards, FM Labs 4-card (Hormone Panel/DUTCH Test/Ultrasound/Nutrient Panel), 6 FAQ, doctor card, 4 refs (PMID17012440 Dawood, PMID22257365 Simoens, PMID16690673 Proctor, PMID16713928 Harel). Note: #6 is 404/no-HTML, different slug — no overlap. SEO: 8.5/10. Plagiarism: original ✅. Hero: menstrual-pain-hero-1200x630.jpg ⚠️ upload needed. Purple/violet gradient. |
| 27 | ผื่นลมพิษ แก้ไม่หายขาด | `post/ผื่นลมพิษ` | 67 | — | ✅ HTML พร้อม | `blog-urticaria-hives.html` — ~3,075 words, 6 H2, 1 table (Urticaria Types 7-row), 4 stat cards (20%/1%/50%+/3:1 gender), Mast Cell Degranulation 5-step mechanism (Sensitization/Re-exposure/Degranulation/Histamine Actions/Late-Phase), Histamine Intolerance section (DAO/High-Histamine Foods/Liberators), Leaky Gut → Urticaria 4-card, 5R Protocol highlight, Thyroid Autoimmunity section, H. pylori section, Angioedema warning, 6 Anti-Urticaria Nutrition cards (Quercetin/Vit C/Omega-3/Probiotics/Zinc/Nettle), MCAS section, FM Labs 4-card (IgE+IgG/Histamine+DAO/Gut Panel/Autoimmune+Thyroid), Tryptase section, 6 FAQ, doctor card, 4 refs (PMID35289967 Zuberbier 2022, PMID27296062 Kolkhir 2016, PMID30866037 Zhao 2019, PMID17490952 Maintz 2007). SEO: 8.5/10. Plagiarism: original ✅. Hero: urticaria-hives-hero-1200x630.jpg ⚠️ upload needed. Teal/cyan gradient. |
| 28 | Triglyceride ค่าที่ไม่ควรมองข้าม | `post/triglyceride` | 64 | 1 | ✅ HTML พร้อม | `blog-triglyceride.html` — ~3,084 words, 6 H2, 2 tables (TG Reference Range 5-row + [implied from Lipid info]), 4 stat cards (31%/&lt;100 mg/dL/50%/5×), TG Metabolism 5-step flow (Digestion/Chylomicron/DNL/VLDL Cascade/HDL Exchange), TG vs Cholesterol 2-card, TG Reference Range table, TG/HDL Ratio guide, Non-Fasting TG highlight, 6 cause cards (Fructose/Refined Carbs/Alcohol/IR/Hypothyroidism/Drugs), Low-Carb strategy, Omega-3/Berberine/IF/Low-Carb treatment cards, Foods to Avoid list, Exercise section, Uric Acid connection, GGT NAFLD section, FM Labs 4-card (Lipid Panel/IR Panel/Liver Markers/Thyroid+Hormone), 6 FAQ, doctor card, 4 refs (PMID21502576 Miller 2011, PMID20160162 Nordestgaard 2010, PMID30415628 Bhatt 2019, PMID10953035 Ginsberg 2000). SEO: 8.5/10. Plagiarism: original ✅. Hero: triglyceride-hero-1200x630.jpg ⚠️ upload needed. Rose/crimson gradient. 1 backlink preserved. |
| 29 | สาเหตุผมร่วงจากฮอร์โมน | `post/hairloss` | 63 | 2 | ✅ Content พร้อม (2026-08-02) | `docs/blog/hairloss.md` — ~2,600 words (Thai), reclaims slug from 404 Wix post (preserves 63 clicks / 2 backlinks). Target keyword: "ตรวจฮอร์โมนผมร่วง ที่ไหนดี" (scored 14/15 via keyword-scout — thin/generic competition, direct service link). 9 H2 sections incl. age-banded hormone-cause table (20-30/30-40/40-50/50+), hero image only (no inline images, per user request), doctor quote (หมอนุ่น), 5 FAQ, 7 refs (StatPearls NBK430924, PMID26500992 Malkud 2015, PMID37692605 Hussein 2023, PMID30785992 Carmina 2019 JCEM, PMID40318238 Gupta 2025 Maturitas, PMID12269873 Pathomvanich 2002 Bangkok, Siriraj Medical Journal). CTA: `/hormones-quiz` + `/personalized-vitamins`. Plagiarism: original synthesis, no source paraphrased ✅. Hero: `hairloss-hero.webp` ✅ delivered (licensed stock photo, user-sourced, center-cropped to 1200×630, "healthy hair after" framing) — ⚠️ Sanity import pending. |
| 30 | 5 วิตามินเสริมภูมิคุ้มกัน | `post/boost-ภูมิคุ้มกัน` | 63 | — | ⚠️ 404 on Wix — page not found, no HTML file. Keep in list for reference. |
| 31 | NAD Plus คืออะไร | `nad` | 59 | 1 | Service page — very important. Keep. |
| 32 | โครเมียม ลดความอยากน้ำตาล | `post/chromium` | 54 | — | ✅ HTML พร้อม | `blog-chromium.html` — ~3,006 words, 6 H2, 2 tables (Food Sources 8-row + AI/Therapeutic Range 6-row), 4 stat cards (90%+/7×/2.8%/35µg), Chromodulin 4-step mechanism (Receptor Amplification/GLUT4 Translocation/Hepatic Glucose Suppression/Brain Insulin), Vicious Cycle paragraph, 3 Form cards (Picolinate/GTF/Polynicotinate), deficiency risk 4-cards (Sugar/Athletes/Stress/Elderly), Chromium+Magnesium+B6 Synergy highlight, PMOS section, Gestational Diabetes section, timing & interaction notes, FM Labs 4-card (Serum+RBC Chromium/Urine Chromium/IR Panel/Micronutrient Panel), 6 FAQ, doctor card, 4 refs (PMID9366247 Anderson 1997, PMID22892049 Abdollahi 2013, PMID18556586 Anton 2008, PMID20410986 Vincent 2010). SEO: 8.5/10. Plagiarism: original ✅. Hero: chromium-hero-1200x630.jpg ⚠️ upload needed. Indigo/violet gradient. |
| 33 | จะเกิดอะไรขึ้นเส้นเลือดตีบ | `post/cloggedarteries` | 51 | — | ✅ HTML พร้อม | `blog-clogged-arteries.html` — ~3,297 words, 6 H2, 2 tables (Organ Impact 6-row + Advanced CV Risk Assessment 8-row), 4 stat cards (#1/50%+/20s/70%), Atherosclerosis 5-step flow (Endothelial Dysfunction/LDL Oxidation+Foam Cells/Fibrous Cap/Plaque Growth+Calcification/Plaque Rupture), 8-risk-factor grid (Insulin Resistance/Inflammation/Oxidative Stress/Hypertension/Dyslipidemia/Homocysteine/Smoking/Genetics), 6-symptom grid, hs-CRP vs LDL comparison, CAC Score highlight, 6-micronutrient grid (Omega-3/K2/Magnesium/D3+K2/CoQ10/Berberine), Methylated B Vitamins section, FM Labs 4-card (Advanced Lipid/Inflammation+Oxidative Stress/IR Panel/Micronutrient), 6 FAQ, doctor card (หมอนุ่น), 4 refs (PMID21593864 Libby 2011, PMID18997196 Ridker 2008 JUPITER, PMID18362249 Meyers 2008, PMID25282031 Mortensen 2014). SEO: 8.5/10. Plagiarism: original ✅. Hero: clogged-arteries-hero-1200x630.jpg ⚠️ upload needed. Blue/navy gradient. |
| 34 | แฟนไม่ได้หมดรัก สารสื่อประสาท | `post/neurotransmitter` | 41 | — | ✅ HTML พร้อม | `blog-love-neurotransmitter.html` — ~3,001 words, 6 H2, 1 table (6 Neurotransmitters/Hormones: บทบาทในความรัก/ผลเมื่อขาด/ปัจจัยที่ลดระดับ), 4 stat cards (18-24mo/20sec/3ระยะ/50%+), Love 3-phase flow (Lust/Attraction/Attachment with Dopamine→Oxytocin shift), Cortisol-Oxytocin inverse relationship, Vicious Conflict Cycle highlight, Gut-Brain Axis + Neuroinflammation paragraph, 6-food grid (Tryptophan/Tyrosine/Probiotics/Omega-3/Magnesium/Zinc), 4-Oxytocin-Boosting activities grid (Touch/Sex/Novel Experience/Attunement), 7-Lifestyle tips (Sleep/Zone2 Exercise/Mindfulness/Nature/Screen Time/Sunlight/Social), DUTCH Test/Burnout-Attachment paragraph, FM Labs 4-card (Neurotransmitter Profile/Hormone Panel/Micronutrient/Gut+Amino Acid), 6 FAQ, doctor card, 4 refs (PMID16255006 Fisher 2005, PMID24050183 Carter 2014, PMID11439708 Kiecolt-Glaser 2001, PMID23759244 Dinan 2013 Psychobiotics). SEO: 8.5/10. Plagiarism: original ✅. Hero: love-neurotransmitter-hero-1200x630.jpg ⚠️ upload needed. Rose/pink gradient. |

---

### Bucket 2 — REWRITE FOR SEO/GEO 🔄 กำลังทำ (36 posts)

| # | Post Title (TH) | URL Slug | Visits/mo | Backlinks | สถานะ | Notes |
|---|----------------|----------|----------:|----------:|-------|-------|
| 35 | ระบบภูมิคุ้มกัน Immune System | `immunesystem` | 20 | 201 | ✅ HTML พร้อม | `blog-immune-system.html` — ~6,400 words, 7 sections, 6 FAQ, 5 refs. Plagiarism check: original ✅. Hero image needed: `immune-system-hero-1200x630.jpg` |
| 36 | โกรทฮอร์โมน (Growth Hormone) ลดลง | `post/growth-hormone-2` | 1 | 108 | ✅ HTML พร้อม | `blog-growth-hormone-decline.html` — ~3,091 words, 7 H2, 3 H3, 2 tables, 6 FAQ, 5 refs. SEO: 8.4/10. Plagiarism: original ✅. Toprank: ✅. Hero image needed: `growth-hormone-decline-hero-1200x630.jpg` |
| 37 | Mood Swings | `health-concerns/mood-swings` | 37 | — | ✅ HTML พร้อม | `blog-mood-swings.html` — ~3,929 words, 7 H2, 4 H3, 2 tables, 6 FAQ, 5 refs. SEO: ✅. Plagiarism: original ✅. Toprank: ✅. Hero image needed: `mood-swings-hero-1200x630.jpg` |
| 38 | นวดประคบร้อนแบบไทยๆ | `post/herbal-compress-massage` | 36 | 1 | ✅ HTML พร้อม | `blog-herbal-compress-massage.html` — ~3,100 words, 8 H2, 3 tables, 6 FAQ, 5 refs. SEO: 8.4/10. Plagiarism: original ✅. Toprank: ✅. Hero image: `herbal-compress-massage-hero-1200x630.jpg` ✅ |
| 39 | คู่มือทานวิตามินซี | `post/คู่มือทานวิตามินซี` | 36 | — | ✅ HTML พร้อม | `blog-vitamin-c-guide.html` — ~2,800 words, 9 H2, 3 tables, 6-type grid, 6 FAQ, 5 refs. SEO: 8.5/10. Plagiarism: original ✅. Toprank: ✅. Hero: vitamin-c-guide-hero-1200x630.jpg ✅ |
| 40 | แพ้กุ้ง แพ้แป้ง IgE | `post/food-allergy-ige` | 35 | — | ✅ HTML พร้อม | `blog-allergy-ige.html` — ~3,000 words, 8 H2, 3 tables, severity grid, Big 8 grid, 6 FAQ, 5 refs. PMC evidence: Tropomyosin 68.8%, Omega-5 Gliadin 100% WDEIA. SEO: 8.5/10. Plagiarism: original ✅. Toprank: ✅. Hero: allergy-ige-hero-1200x630.jpg ✅ |
| 41 | แมกนีเซียม ลดปวดไมเกรน | `post/magnesium` | 34 | — | ✅ HTML พร้อม | `blog-magnesium.html` — ~3,200 words, 9 H2, 2 tables, 5-type grid, 7 symptom cards, 5 benefits, 6 FAQ, 5 refs. PMC evidence: CSD migraine (PMC8912646), sleep RCT p=0.002 (PMC3703169), bisglycinate RCT p=0.049 (PMC12412596). SEO: 8.7/10. Plagiarism: original ✅. Toprank: ✅. Hero: magnesium-hero-1200x630.jpg ✅. Merged #41+#42+#43. |
| 42 | Magnesium ช่วยนอนหลับ | `post/magnesium-1` | 26 | — | ✅ Merged → #41 | Merged into blog-magnesium.html — canonical post/magnesium. |
| 43 | Magnesium แร่ธาตุฮีโร่ | `post/magnesium-hero` | 2 | — | ✅ Merged → #41 | Merged into blog-magnesium.html — canonical post/magnesium. |
| 44 | 5 สัญญาณผิวเตือนภูมิคุ้มกันพัง | `post/ภูมิคุ้มกันพัง` | 29 | 2 | ✅ HTML พร้อม | `blog-skin-immunity.html` — ~3,000 words, 5 H2 signs, 5 H3 mechanisms, 2 tables, 5-card grid, 6 FAQ, 5 refs (PMC6450064, PMC7215310, PMC8470589, PMC5793244, PMC4098727). SEO: 8.8/10. Plagiarism: original ✅. Toprank: ✅. Hero: skin-immunity-hero-1200x630.jpg ✅. Teal/cyan gradient. |
| 45 | วิตามินบี6 สำหรับคุณแม่ตั้งครรภ์ | `post/vitamin-b6` | 26 | — | ✅ HTML พร้อม | `blog-vitamin-b6.html` — ~3,000 words, 7 H2, 2 tables, 6-card grid, 5-step list, 6 FAQ, 5 refs (PMID7485315, PMID10334745, PMC5470355, PMC8469759, PMC2077351). SEO: ✅. Plagiarism: original ✅. Toprank: ✅. Hero: vitamin-b6-hero-1200x630.jpg ✅. Warm amber-gold gradient. |
| 46 | Gut 3P ดีต่อลำไส้ | `post/postbiotic` | 25 | — | ✅ HTML พร้อม | `blog-gut-3p.html` — ~3,100 words, 6 H2, 3P card grid, SCFA grid, 2 tables, chip lists, 6 FAQ, 5 refs (PMC8387233 ISAPP postbiotics, PMC6522433 prebiotics, PMC4326510 probiotics, PMC3070119 butyrate, PMC4056765 microbiota immunity). SEO: 8.7/10. Plagiarism: original ✅. Toprank: ✅. Hero: probiotic-foods-gut-health-hero-1200x630.jpg ✅. Forest green gradient. |
| 47 | สเตียรอยด์ ที่เราสร้างได้เอง | `post/สเตียรอยด์` | 24 | — | ✅ HTML พร้อม | `blog-steroid-cortisol.html` — ~3,100 words, 6 H2, 4-card steroid grid, Cortisol diurnal rhythm bar, DHEA age-decline table, chip lists, Cushing's/Addison's sections, 5-step balance list, 6 FAQ, 5 refs (PMID19488073 Chrousos, PMID12467951 Tsigos, PMC2386281 Nieman Cushing's, PMID12788587 Arlt Addison's, PMID10760297 Baulieu DHEA). SEO: ✅. Plagiarism: original ✅. Toprank: ✅. Hero: steroid-cortisol-hero-1200x630.jpg ✅. Warm copper/sienna gradient. |
| 48 | สาเหตุและอาการ PID อุ้งเชิงกราน | `post/pelvicinflammatorydisease` | 21 | 3 | ✅ HTML พร้อม | `blog-pid.html` — ~3,000 words, 7 H2, 4-card organ grid, 4-step diag grid, bacteria table, complication stats (6× ectopic, 14× infertility), chip lists, 5-step treatment list, 6 FAQ, 5 refs (PMID25988104 Brunham, PMC5052677 Curry, Westrom 1975, Haggerty 2006, CDC 2021). SEO: ✅. Plagiarism: original ✅. Toprank: ✅. Hero: pid-hero-1200x630.jpg ✅. Deep rose/magenta gradient. |
| 49 | ประจำเดือนมามากผิดปกติ | `post/menorrhagia` | 20 | — | ✅ HTML พร้อม | `blog-menorrhagia.html` — ~3,100 words, 6 H2, PALM-COEIN grid, practical-test box, bacteria/cause table, 6-card iron-deficiency grid, 5-step diag flow, treatment comparison table, 6 FAQ, 5 refs (PMID22023087 Munro PALM-COEIN, PMC3609940 Philipp VWD, Lethaby Cochrane LNG-IUS, PMID20859150 Lukes tranexamic, ACOG PB128). SEO: ✅. Plagiarism: original ✅. Hero: menorrhagia-hero-1200x630.jpg ✅. Purple/plum gradient. |
| 50 | วิตามินบี3 ปัญหาผิวหย่อยคล้อย | `post/vitamin-b3` | 18 | — | ✅ HTML พร้อม | `blog-vitamin-b3.html` — ~10,000+ words, 7 H2, 8 H3, 6-card benefit grid, NAD+ age-decline bar chart, Pellagra 4D grid, 2 tables (food sources + RDA), 5-step supplement guide, 6 FAQ, 5 refs (PMID16029679 Bissett aging skin, PMC8389214 Rolfe niacinamide mechanisms, PMID18492135 Bissett yellowing/wrinkling, PMID26785476 Verdin NAD+ aging, PMC11727686 serum RCT). SEO: ✅. Plagiarism: original ✅. Toprank: ✅. Hero: vitamin-b3-hero-1200x630.jpg ✅. Deep wine/ruby gradient. |
| 51 | 8 ลักษณะสุขภาพดี | `post/8-ลักษณะสุขภาพดี` | 18 | — | ✅ HTML พร้อม | `blog-8-healthy-characteristics.html` — ~9,600 words, 7 H2, 8 listicle cards (sleep, energy, digestion, skin/hair/nails, immunity, mood, weight, lab values), summary table, 6 FAQ, 5 refs (PMID29073412 NSF sleep, PMID19805654 metabolic syndrome, PMID30920354 sleep-immune, PMID31460832 gut-brain axis, PMC6380979 hair vitamins). SEO: ✅. Plagiarism: original ✅. Toprank: ✅. Hero: 8-healthy-characteristics-hero-1200x630.jpg ✅. Sky blue/cerulean gradient. |
| 52 | สาวๆ อยากมีกล้าม Testosterone | `post/testosterone-woman` | 17 | — | ✅ HTML พร้อม | `blog-testosterone-women.html` — ~3,024 words, 10 H2, 1 table, 3-source grid, 7-symptom grid, mechanism box, men-vs-women table, 6 natural boost tips, DHEA/test protocol, 6 FAQ, 5 refs (PMC6893124, PMID36585396, PMC12456395, PMID36577241, PMID26358173). SEO: ✅. Plagiarism: original ✅. Hero: testosterone-women-hero-1200x630.jpg ✅. Terracotta/brick gradient. |
| 53 | สารกันบูดในอาหาร | `post/preservatives` | 16 | 3 | ✅ HTML พร้อม | `blog-preservatives.html` — 3,000 words, 11 H2, 1 table, 5-type preservative grid, harm grid, E-number guide, ADI explainer, detox pathway, children section, 6-tip grid, 6 FAQ, 5 refs (PMID17825405 McCann Lancet 2007, PMID26514947 IARC processed meat, PMID8493816 BHA/BHT, PMID7673519 sulfite sensitivity, PMC8399985 food additives gut). Plagiarism: original ✅. Hero: preservatives-hero-1200x630.jpg ⚠️ upload needed. Dark amber/ochre gradient. |
| 54 | เช็คภูมิคุ้มกัน ภูมิตก! | `post/check-immune-system` | 16 | — | ✅ HTML พร้อม | `blog-check-immune-system.html` — 3,000 words, 12 H2, 1 table, 10-sign grid, 3-layer grid, test table, 6-nutrient grid, 7-step restoration list, Thai foods section, recovery timeline, supplement list, 6 FAQ, 5 refs (PMID19139325 Cohen sleep, PMC3738984 Prietl Vit D, PMC2723580 Haase Zinc, PMC6523821 Nieman exercise, PMC3845678 Mayer gut). Plagiarism: original ✅. Hero: check-immune-system-hero-1200x630.jpg ⚠️ upload needed. Navy/cobalt gradient. |
| 55 | ฮอร์โมนเพศหญิง ที่ต้องรู้จัก | `post/femaleshormones` | 15 | — | ✅ HTML พร้อม | `blog-female-hormones.html` — ~3,500 words, 10 H2, 8 H3, 1 table (testing schedule), 5-hormone card grid, 4-phase cycle timeline, 8-sign imbalance grid, age bar chart, 6-tip grid, 6 FAQ, 5 refs (PMID14667980 Stanczyk, PMID26358173 Davis, PMC3264051 Plottel Estrobolome, PMID25905282 Reed Reed, PMID15288182 Kalantaridou). SEO: 7.8/10. Plagiarism: original ✅. Toprank: ✅. Hero: female-hormones-hero-1200x630.jpg ⚠️ upload needed. Deep rose/violet gradient. |
| 56 | 8 วิธีเพิ่ม Human Growth Hormone | `post/human-growth-hormone` | 14 | — | ✅ HTML พร้อม | `blog-hgh-natural-boost.html` — ~3,100 words, 6 H2, 6-card HGH function grid, Somatopause decline chart, 8-way numbered list, food chips, testing table (IGF-1), 6 FAQ, 5 refs (PMID3127426 Ho 1988, PMID2355952 Rudman 1990, PMID12137173 Stokes sprint, PMID8491152 Corpas, PMID12086880 Chromiak amino acids). SEO: ✅. Plagiarism: original ✅. Hero: hgh-natural-boost-hero-1200x630.jpg ⚠️ upload needed. Deep teal/emerald gradient. |
| 57 | 6 คุณประโยชน์ Kombucha | `post/6_benefits_kombucha` | 14 | — | ✅ HTML พร้อม | `blog-kombucha-benefits.html` — ~3,100 words, 6 H2, SCOBY composition box, 6-benefit card grid, organic acids table (6 acids), drinking guide, warning box, choosing guide, 6 FAQ, 5 refs (PMC8347189, PMID30670289 Kapp 2019, Jayabalan 2014, Essawet 2015, PMID34698580 gut microbiota). SEO: ✅. Plagiarism: original ✅. Hero: kombucha-benefits-hero-1200x630.jpg ⚠️ upload needed. Forest green/lime gradient. |
| 58 | หน้ามืด วูบบ่อย อันตราย | `post/syncope` | 12 | — | ✅ HTML พร้อม | `blog-syncope-fainting.html` — ~3,200 words, 6 H2, 4-step mechanism timeline, 3-type danger cards (Vasovagal/Orthostatic/Cardiac), red flags section (8 signs), hormone connection table, 6 prevention tips, 6 FAQ, 5 refs (PMID29562304 Brignole 2018 ESC, PMID12239256 Soteriades NEJM 2002, PMID21193201 Freeman, PMID19713422 Moya 2009, PMID26477549 Wieling). SEO: ✅. Plagiarism: original ✅. Hero: syncope-fainting-hero-1200x630.jpg ⚠️ upload needed. Slate blue/indigo gradient. |
| 59 | นอนละเมอ สาเหตุ | `post/sleepwalking` | 12 | — | ✅ HTML พร้อม | `blog-sleepwalking.html` — ~3,200 words, 6 H2, sleep architecture stage grid, mechanism box (4 steps), 8-trigger card grid, complexity scale (simple/complex/dangerous), hormone table, treatment cards (6 steps), safety guide, testing table, 6 FAQ, 5 refs (PMID25938617 Petit 2015, PMID23415567 Zadra 2013, PMC5096813 Stallman 2016, PMID17208473 Pressman, PMID18465379 Lam). SEO: ✅. Plagiarism: original ✅. Hero: sleepwalking-hero-1200x630.jpg ⚠️ upload needed. Deep indigo/midnight gradient. |
| 60 | Burn Out Syndrome Adrenal Fatigue | `how-to-overcome-burn-out-syndrome` | 12 | — | ✅ HTML พร้อม | `blog-burnout-adrenal.html` — ~3,300 words, 7 H2, WHO ICD-11 3-dim box, Burnout vs Stress compare grid, 4-stage timeline, HPA Axis dysregulation flow, Cortisol diurnal pattern compare, symptoms table (7 systems), recovery grid (6 cards), nutrition grid (6 micronutrients), testing table (7 tests), 6 FAQ, 5 refs (WHO ICD-11, PMID27265691 Maslach 2016, PMID16248680 Toker, Witt 2017 BMC, PMID20888369 Clow CAR). SEO: ✅. Plagiarism: original ✅. Hero: burnout-adrenal-hero-1200x630.jpg ⚠️ upload needed. Amber/rust gradient. |
| 61 | NK Cell เม็ดเลือดขาว | `post/nkcell` | 3 | — | ✅ HTML พร้อม | `blog-nk-cell.html` — ~8,867 words, 9 H2, 5 H3, 6 FAQ, 5 refs (PMID21212348, PMID31289370, PMID15250815, PMID31193280, PMC3738984). SEO: ✅. Plagiarism: original ✅. Hero: nk-cell-blog-hero-1200x630.jpg ⚠️ upload needed. Purple gradient. |
| 62 | Progesterone ฮอร์โมนเพศหญิง | `post/progesterone` | 8 | 8 | ✅ HTML พร้อม | `blog-progesterone.html` — ~8,490 words, 10 H2, 4 H3, 6 FAQ, 5 refs (PMC3987489, PMID24184390, PMID10432133, PMID18583056, Romm 2017). SEO: ✅. Plagiarism: original ✅. Hero: progesterone-hero-1200x630.jpg ⚠️ upload needed. Rose/mauve gradient. ⭐ 8 backlinks — high authority. |
| 63 | ฮอร์โมนวัยทอง เริ่มตอนอายุเท่าไหร่ | `post/ฮอร์โมนวัยทอง` | 9 | — | ✅ HTML พร้อม | `blog-menopause-hormones.html` — ~3,200 words, 8 H2, 3 H3, 3-phase grid (Peri/Meno/Postmeno), 6 hormone cards, 4-stage timeline, symptom grids, hormone testing table, HRT comparison table, 6 FAQ, 5 refs (PMID22343510, PMID26444994, PMID12117397, PMC1199595, PMID27681935). SEO: ✅. Plagiarism: original ✅. Hero: menopause-hormones-hero-1200x630.jpg ⚠️ upload needed. Rose gradient. |
| 64 | ตรวจภูมิแพ้อาหารแฝง IgG | `post/ตรวจภูมิแพ้อาหารแฝง-igg` | 2 | — | ✅ HTML พร้อม | `blog-food-intolerance-igg.html` — ~3,100 words, 8 H2, 3 H3, IgE vs IgG compare grid, symptom grid, food trigger table, 5-step process, Leaky Gut section, 6 FAQ, 5 refs (PMID10895521, PMID23981825, PMID20203504, PMID18070223, PMC6893528). SEO: ✅. Plagiarism: original ✅. Hero: food-intolerance-igg-hero-1200x630.jpg ⚠️ upload needed. Teal gradient. |
| 65 | PMS อาการก่อนเป็นประจำเดือน | `post/pms-premenstrualsyndrome` | 1 | — | ✅ HTML พร้อม | `blog-pms.html` — ~3,100 words, 9 H2, 4-phase cycle timeline visual, physical + emotional symptom grids, 6 cause cards, PMS vs PMDD table, supplement tip grid, 6 FAQ, 5 refs (PMID18395582, PMID9731851, PMID10682869, PMC27878, PMID29095191). SEO: ✅. Plagiarism: original ✅. Hero: pms-hero-1200x630.jpg ⚠️ upload needed. Purple gradient. |
| 66 | PMOS สาเหตุและอาการ | `post/_pcos` | 10 | — | ✅ HTML พร้อม | `blog-pcos.html` — ~3,200 words, 9 H2, Rotterdam Criteria grid (3-criterion), symptom grid, 6 mechanism cards, diagnostic table, treatment sections (lifestyle/supplements/meds), risk box, 6 FAQ, 5 refs (PMID14711538, PMID18950759, PMID9626083, PMC5655679, PMID30052961). SEO: ✅. Plagiarism: original ✅. Hero: pcos-hero-1200x630.jpg ⚠️ upload needed. Violet gradient. |
| 67 | อินซูลิน ฮอร์โมนป้องกันเบาหวาน | `post/insulin` | 11 | — | ✅ HTML พร้อม | `blog-insulin.html` — ~3,000 words, 8 H2, 6 insulin function cards, 5-step mechanism cascade, symptom grid, IR diagnostic table, Fasting Glucose info box, diet/exercise/supplement sections, 6 FAQ, 5 refs (PMID2044434, PMID8039601, PMID18442638, PMID10683091, PMID29378044). SEO: ✅. Plagiarism: original ✅. Hero: insulin-hero-1200x630.jpg ⚠️ upload needed. Sky blue gradient. |
| 68 | ภาวะต่อมหมวกไตล้า Adrenal Fatigue | `adrenal-fatigue` | 7 | 2 | ✅ HTML พร้อม | `blog-adrenal-fatigue-post.html` — ~3,200 words, 8 H2, 4 adrenal hormone cards, Cortisol rhythm bar chart (5 time points normal vs dysregulated), symptom grid, 4-stage HPA dysregulation table, testing section, Adrenal Fatigue vs Addison's box, recovery program, 6 FAQ, 5 refs (PMID19488073, PMID12377295, PMID20369498, PMID12788587, BMC 2017). SEO: ✅. Plagiarism: original ✅. Hero: adrenal-fatigue-post-hero-1200x630.jpg ⚠️ upload needed. Red/rose gradient. ⭐ 2 backlinks. |
| 69 | Coenzyme Q10 หัวใจ | `post/coenzyme-q10` | 10 | 1 | ✅ HTML พร้อม | `blog-coq10.html` — ~3,200 words, 7 H2, 7-benefit card grid, Ubiquinone vs Ubiquinol compare, CoQ10 age-decline bar chart (5 points), Statin mechanism section, food table, dosage table, 6 FAQ, 5 refs (PMID15860267 Langsjoen, PMID24177285 Q-SYMBIO, PMC3136776 Statin myopathy, PMID16230184 Rosenfeldt BP, PMID17914161 Littarru). SEO: ✅. Plagiarism: original ✅. Hero: coq10-hero-1200x630.jpg ⚠️ upload needed. Amber/gold gradient. ⭐ 1 backlink. (#12 is 404 on Wix — no overlap.) |
| 70 | Vitamin D สำหรับภูมิคุ้มกัน | `post/vitamin-d-immune` | — | — | ✅ HTML พร้อม | `blog-vitamin-d-immune.html` — ~3,300 words, 8 H2, 6 immunity pathway cards, Thai deficiency causes table, symptom grid (6 cards), 25(OH)D level table with badges, food table, dosage table, Myth vs Fact grid, 6 FAQ, 5 refs (PMC3738984 Prietl, PMID22264449 Holick, PMID20219962 Bergman, PMC5310969 Martineau BMJ 2017, PMID21646368 Endocrine Society guideline). SEO: ✅. Plagiarism: original ✅. Hero: vitamin-d-immune-hero-1200x630.jpg ⚠️ upload needed. Solar gold/amber gradient. |

---

### Bucket 3 — DELETE / DO NOT MIGRATE 🗑️ (17 posts)

| # | Post Title (TH) | URL Slug | Issues | Notes |
|---|----------------|----------|--------|-------|
| 71 | All /blog/hashtags/ pages | `blog/hashtags/*` | 404 | Wix auto-generated. Do not migrate. |
| 72 | All /members-area/ spam pages | `members-area/*` | 404 + spam | Spam bot registrations. Ignore completely. |
| 73 | All /blog/categories/ pages | `blog/categories/*` | Thin <200w | Auto-generated, no meta. WordPress generates better. |
| 74 | /pricing-plans/list | `pricing-plans/list` | 35 words | Wix-specific. Do not migrate. |
| 75 | Service pages (/service-page/) | `service-page/*` | <50 words | Wix booking pages. Already rebuilt in new HTML. |
| 76 | 6สูตร Buffet IV Drip Vitamin | `post/6สูตรbuffet-iv-drip-vitamin` | 124 words | Too thin. Rewrite as new post instead. |
| 77 | Booster Quercetin ต้านไวรัส | `post/booster-to-quercetin` | Outdated COVID | COVID-era 2021. Delete. |
| 78 | N-Acetylcysteine NAC vs COVID | `post/n-acetylsysteine-nac-against-covid-19` | Outdated COVID | COVID-era. Delete. |
| 79 | 7 วิตามินเสริมภูมิ (duplicate 1) | `post/7-วิตามินเสริมภูมิคุ้มกัน` | Duplicate | Nearly identical to another post. Merge or delete. |
| 80 | 7 วิตามินเสริมภูมิ (duplicate 2) | `post/7-วิตามินเสริมภูมิ-มีอะไรบ้าง` | Duplicate | Second duplicate. Delete. |
| 81 | Healthy Food Series (ตอนที่ 1-5,7) | `post/healthy-food-series` | <200w each | Too thin. Merge into one post or delete. |
| 82 | นอนไม่หลับ GABA (sleep-gaba) | `post/sleep-gaba` | Dup of #16 | ✅ MERGED into blog-gaba.html (#16) — canonical = post/_gaba. Redirect post/sleep-gaba → post/_gaba when deploying. |
| 83 | โยคะฟลาย Yoga Fly | `post/yogafly` | Off-topic | Not related to Thrive services. Delete. |
| 84 | /white-skin | `white-skin` | 404 | Page doesn't exist. Do not migrate. |
| 85 | ตรวจภูมิแพ้ Antibody COVID (IgG) | `igg-antibody-covid` | Outdated | COVID antibody test. Delete or update entirely. |
| 86 | การกอดที่มากกว่าการบอกรัก | `post/__hug` | Off-topic | Lifestyle fluff — no relevance to clinic. Delete. |
| 87 | ลำไส้อักเสบ (broken URL) | `post/ลำไส้อักเสบ (404)` | 404 | URL encoding broken. Do not migrate. |

---

## Standard Checklist สำหรับทุกหน้าใหม่

ก่อน commit ทุกครั้ง ตรวจสอบ (**ใช้กับทั้ง Tier A และ Tier B Blog**):

### Meta Tags
- [ ] `<title>` — keyword หลัก + brand + location (ไม่เกิน 60 chars)
- [ ] `<meta name="description">` — keyword + CTA + เบอร์ (ไม่เกิน 155 chars)
- [ ] `<link rel="canonical">`
- [ ] `<meta name="robots" content="index, follow">`
- [ ] OG tags ครบ 7 รายการ
- [ ] Twitter Card tags ครบ 4 รายการ
- [ ] GEO tags ครบ 4 รายการ
- [ ] Hreflang: th-TH, en, x-default

### JSON-LD Schema
- [ ] MedicalClinic (ใช้ @id เดิม `#clinic`)
- [ ] Person — หมอนุ่น (ใช้ @id เดิม `#dr-chanakan`)
- [ ] หน้าที่เหมาะสม: FAQPage, MedicalTest, MedicalProcedure, BreadcrumbList, Article

### Images (ตรวจก่อนเริ่มเขียน HTML ทุกครั้ง)
- [ ] `[topic]-hero-1200x630.jpg` — วางใน `image/`
- [ ] `dr-chanakan-trangansri-thrive-400x400.jpg` — มีอยู่แล้วเสมอ ✅

### Content
- [ ] Hero image + hook headline
- [ ] Doctor section + รูปหมอนุ่น → **อัปเดตตาราง Doctor on File ใน SITE-TRACKER ทันที**
- [ ] FAQ section (อย่างน้อย 5 คำถาม)
- [ ] อ้างอิง PubMed อย่างน้อย 4 รายการ
- [ ] CTA ครบ: โทร + LINE ทั้ง hero, mid-page, final
- [ ] Mobile sticky bar
- [ ] Internal links ไปหน้า service ที่เกี่ยวข้อง (4 cards)
- [ ] ข้อมูลคลินิกใน footer ถูกต้อง
- [ ] Copy ไปยัง `new html from vkasama/` ก่อน commit

### Content Quality (Bucket 2 Blog เพิ่มเติม)
- [ ] Word count ≥ 3,000 คำ (ตรวจด้วย script หรือนับ chars ÷ 3)
- [ ] มี H2 อย่างน้อย 5 หัวข้อ
- [ ] มี Table หรือ Comparison อย่างน้อย 1 รายการ
- [ ] เนื้อหาครอบคลุม search intent — ตอบคำถามหลักได้ครบ

### ✅ Toprank SEO/GEO Flow (บังคับทุก Blog — ทำก่อนเขียน)
1. **`/toprank keyword-research`** — หา Thai search terms จริงสำหรับหัวข้อ → ใช้ใน title, H2, FAQ
2. **`/toprank geo-optimizer`** — วาง GEO signals, local schema, location intent ก่อนเขียน

### ✅ Toprank SEO Audit (บังคับทุก Blog — ทำหลังเขียน)
3. **`/toprank seo-page`** — audit เนื้อหาที่เขียนแล้วเทียบ E-E-A-T, on-page SEO, search intent
4. **`/toprank meta-tags-optimizer`** — ตรวจ title, description, OG tags ให้ได้มาตรฐาน

### ✅ Plagiarism Check (บังคับทุกหน้า — ทำหลัง toprank audit)
5. **Web search check** — ค้น 2–3 ประโยคเด่นจากเนื้อหาใน Google เพื่อตรวจว่าไม่ซ้ำกับเว็บอื่น
6. **Phrase originality** — ตรวจว่า analogy, doctor quote, และ section framing เป็นของ Thrive เอง
7. **Free tool** (optional แต่แนะนำ):
   - [Quetext](https://www.quetext.com) — รองรับภาษาไทย, 5 checks/วันฟรี
   - [Copyscape](https://www.copyscape.com) — ใช้หลัง publish (ตรวจ URL)
8. **บันทึกผลทั้งหมด** ในตาราง Bucket 2: `Toprank: ✅ | Plagiarism: original ✅` ก่อน commit

---

## Image Naming Convention

```
[topic-slug]-hero-1200x630.jpg       ← hero / OG image
[topic-slug]-content-800x600.jpg     ← in-page content image
dr-[name]-thrive-400x400.jpg         ← doctor profile (square)
dr-[name]-thrive-800x600.jpg         ← doctor profile (wide, about page)
```
